...
try {
    ...
    // Ensure announcements table exists (create if missing)
    ...
    if (!$connection->query($createSql)) {
        echo json_encode(['success' => false, 'message' => 'Failed to ensure announcements table: ' . $connection->error]);
        exit;
    }
    ...
    try {
        ...
        $sql = "INSERT INTO announcements (title, content, announcement_date, end_date, priority, status, created_at)
            VALUES (?, ?, ?, ?, ?, ?, NOW())";
    ...
    if (!$stmt) {
        ...
        $stmt->bind_param('ssssss', $title, $content, $announcement_date, $end_date, $priority, $status);
    if (!$stmt->execute()) {
